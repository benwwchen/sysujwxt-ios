...
class GPATableViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        ...
        jwxt.getGPA(isGetAll: true) { (success, object) in
            if success {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "GPATableViewCell", for: indexPath)
        
        let keys = gpas.keys.sorted()

        if let titleLabel = cell.textLabel,
            let detailLabel = cell.detailTextLabel {
            titleLabel.text = keys[indexPath.row]
            detailLabel.text = gpas[keys[indexPath.row]]
        }
